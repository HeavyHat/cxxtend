...
namespace hh::functools {
  ...
  TEST(lru_cache, invocation_of_cached_method_is_equivialant_to_non_cached_variant) {
    ...
    EXPECT_EQ(cached_add(1, 1), 1 + 1)
        << "Cached evaluation and non cached evaluation is not equivilant with no resuse! It is "
           "likely that the correct underlying implementation is not being called.";
  }

  TEST(
      lru_cache,
      invocation_of_cached_method_is_equivialant_to_non_cached_variant_when_returning_precalculated_value) {
    auto cached_add = hh::functools::make_lrucache(add);
    cached_add(1, 1);
    EXPECT_EQ(cached_add(1, 1), 1 + 1)
        << "Cached evaluation and non cached evaluation is not equivilant with reuse! It is likely "
           "that the cache is malformed.";
  }

  TEST(lru_cache, size_increases_as_unique_parameterised_calls_are_made) {
    auto cached_add = hh::functools::make_lrucache(add);
    cached_add(1, 1);
    EXPECT_EQ(cached_add.size(), 1)
        << "Cached evaluation and non cached evaluation is not equivilant with reuse! It is likely "
           "that the cache is malformed.";
    cached_add(1, 2);
    EXPECT_EQ(cached_add.size(), 2)
        << "Cached evaluation and non cached evaluation is not equivilant with reuse! It is likely "
           "that the cache is malformed.";
    cached_add(2, 1);
    EXPECT_EQ(cached_add.size(), 3)
        << "Cached evaluation and non cached evaluation is not equivilant with reuse! It is likely "
           "that the cache is malformed.";
    cached_add(2, 2);
    EXPECT_EQ(cached_add.size(), 4)
        << "Cached evaluation and non cached evaluation is not equivilant with reuse! It is likely "
           "that the cache is malformed.";
  }
  TEST(lru_cache,
       size_increases_as_unique_parameterised_calls_are_made_but_bot_when_input_is_equal) {
    auto cached_add = hh::functools::make_lrucache(add);
    cached_add(1, 1);
    EXPECT_EQ(cached_add.size(), 1)
        << "Cached evaluation and non cached evaluation is not equivilant with reuse! It is likely "
  ...
  TEST(lru_cache, returns_correct_result_for_smaller_cache_sizes) {
    ...
    EXPECT_THAT(actual, ::testing::ElementsAreArray(expected))
        << "Calling the uncached variant of the function yielded some different results for large "
           "sizes!";
  }
